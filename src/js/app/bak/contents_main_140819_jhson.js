	...
	function ContentsModel(pmid, pkword) {
		...
		self.contents = {
			...
			lec: {
				...
				subBookList: ko.observableArray(),
				subMockList: ko.observableArray(),
				subBookSelected: ko.observable(""),
				subMockSelected: ko.observable(""),
				page: 1,
				...
				showDetail : function(){
					...
					window.scrollTo(0, 0);
					this.subBookSelected("도서 상품을 선택하세요");
					this.subMockSelected("모의고사 상품을 선택하세요");
					try{this.subBookList.removeAll();}catch(e){}
					try{this.subMockList.removeAll();}catch(e){}
					//location.href = "#";
				},
				hideDetail : function(){
			...
			init: function(){
				...
				this.calcLecTotalPrice = ko.computed(function(){
					...
					var iSumSubPrice = 0;
					var aSubBook = self.contents.lec.subBookList();
					var aSubMock = self.contents.lec.subMockList();
					var iLenSubBook = (aSubBook)? aSubBook.length : 0;
					var iLenSubMock = (aSubMock)? aSubMock.length : 0;
