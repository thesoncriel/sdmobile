	...
	function ContentsModel(pmid, pkword) {
		...
		self.contents = {
			...
			onSubmitOrder: function(elem, isBuy){
				...
				$.post(jqFormItem.attr("action"), mParam, function(data){
					//FIXME: 보내고 난 뒤 ORDER VIEW 보여주기
					self.contents.onsubmitorder( $.parseJSON(data), isBuy );
					//console.log( mParam );
					//console.log( "보내기 성공!" );
				});
				...
				if (param.bbsId){
					if (param.subId){
						if (this.jqForm.find("[name='lec_cat_id']").length === 0){
							this.jqForm.append("<input type=\"hidden\" name=\"lec_cat_id\" value=\"" + param.subId + "\"/>");
						}
						
						this.cateText( param.keyword );
						this.search(1, true);
						this.jqForm.remove("[name='lec_cat_id']");
					}
					else{
						this.cateText( "컨텐츠 검색" );
