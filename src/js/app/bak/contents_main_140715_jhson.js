...
define(["jquery","carousel", "knockout", "knockoututils", "json2.min", "bootstrap", "jqueryfinger", "owlcarousel"], function($, carousel, ko, koutil, json, bootstrap) {
		...
		self.contents = {
			...
			onSubmitOrder: function(elem, isBuy){
				...
				$.post(jqFormItem.attr("action"), mParam, function(data){
					...
					if (self.order !== undefined){
						if (isBuy === true){
							self.order.load( self.order.showWithInfo );
						}
						else{
							self.order.load( self.order.show );
						}
	...
	ContentsModel.prototype = new BaseViewModel();
