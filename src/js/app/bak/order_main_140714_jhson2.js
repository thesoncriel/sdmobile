	...
	function OrderModel(pmid, pkword) {
		...
		self.order = {
			...
			init: function(){
				...
				// 슬라이드 이벤트 바인딩
				$("#form_cartList").on("flick", function(e){
					//console.log(e);
					if (e.direction === 1){
						m.slidePrev();
					}
			...
			totalDiscount : ko.computed(function(){
				try{
			...
			excludingShippingPrice : ko.computed(function(){
				try{
			...
			totalSettlePrice : ko.computed(function(){
				try{
			...
			orderName : ko.computed(function(){
				try{
