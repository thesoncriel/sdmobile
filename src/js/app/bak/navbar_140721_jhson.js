...
window.m = window.m || {};
define(["jquery","carousel", "knockout", "knockoututils", "json2.min", "bootstrap", "mobilebase"], function($, carousel, ko, koutil, json, bootstrap) {
	var lecCateInfo = {
		...
		self.nav = {
			...
			showSub : function(data, event){
				...
				$.map(obsarrSubCat, function(item, index){
					...
					if ( lecCateInfo[ sCatId ] !== undefined ){
						item.lec_cat_id = lecCateInfo[ sCatId ].lec_cate;
					}
					else{
						item.lec_cat_id = "";
					}
